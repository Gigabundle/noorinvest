...
// ── In-memory store ───────────────────────────────────────────────────────────
let store = INVESTORS.map(i => ({...i}));
let newUsers = [];
const wait = ms => new Promise(r => setTimeout(r, ms));
const api = {
  phoneLookup: async phone => { await wait(800); const r = store.find(u => u.phone===phone); return r ? {found:true,name:r.name,hasAccount:r.hasAccount} : {found:false}; },
  createAccount: async (phone,email,pw) => { await wait(900); const i=store.findIndex(u=>u.phone===phone); if(i<0)return{ok:false,err:"NOT_FOUND"}; if([...store,...newUsers].some(u=>u.email===email))return{ok:false,err:"EMAIL_TAKEN"}; store[i]={...store[i],email,pw,hasAccount:true}; return{ok:true}; },
  login: async (email,pw) => { await wait(800); const r=[...store,...newUsers].find(u=>u.email===email&&u.pw===pw); return r?{ok:true,name:r.name,id:r.id}:{ok:false}; },
  adminLogin: async (email,pw) => { await wait(800); return email==="admin@noorinvest.ng"&&pw==="Admin@2025"?{ok:true,name:"Ibrahim Usman"}:{ok:false}; },
  register: async data => { await wait(1000); if([...store,...newUsers].some(u=>u.email===data.email))return{ok:false,err:"EMAIL_TAKEN"}; if([...store,...newUsers].some(u=>u.phone===data.phone))return{ok:false,err:"PHONE_TAKEN"}; newUsers.push({...data,hasAccount:true}); return{ok:true}; },
};
